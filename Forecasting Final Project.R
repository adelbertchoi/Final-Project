...
read_data <- function(data = data, freq = 1) {
    ...
    for(i in 1:N) {
        ...
        if (freq == 1){
            ...
            series_name <- paste("Series", series_no, ":",  category, "from", series_start, "to", series_start + series_size - 1, sep = " ")  
        }
        if (freq > 1) {
            ...
            series_name <- paste("Series", series_no, ":",  category, "from", series_start, as.vector(season(series_ts))[1], 
                                 "to", floor(time(series_ts)[series_size]), as.vector(season(series_ts))[series_size], sep = " ")  
        }
